...
var _lastAnnContent = '';     // 上次渲染的注解内容，用来检测变化
  ...
  function appendTimelineCard(ann, msgIdx) {
    ...
    if (ann.agentLog) {
      ...
      // 判断角色：received 以'销售'开头 → 销售发出，以'客户'开头 → 客户回复
      var role = '';
      var roleCls = '';
      if (log.received.indexOf('销售') === 0) { role = '销售发出'; roleCls = 'tl-role-sales'; }
      else if (log.received.indexOf('客户') === 0) { role = '客户回复'; roleCls = 'tl-role-customer'; }
      
      if (log.received) {
        html += '<div class="tl-agent-line tl-agent-received" id="al-recv-' + msgIdx + '">';
        if (role) html += '<span class="tl-role-badge ' + roleCls + '">' + role + '</span>';
        html += '<div class="tl-agent-row"><span class="tl-agent-icon">📩</span><div class="tl-agent-content"><span class="tl-agent-label">收到消息</span><div class="tl-agent-text">' + (log.received) + '</div></div></div></div>';
      }
      if (log.analysis) {
      ...
      if (log.recommendedAction) {
        html += '<div class="tl-agent-line tl-agent-action" id="al-act-' + msgIdx + '" style="display:none"><div class="tl-agent-row"><span class="tl-agent-icon">🎯</span><div class="tl-agent-content"><span class="tl-agent-label">推荐动作</span><div class="tl-agent-text">' + (log.recommendedAction) + '</div></div></div></div>';
      }
      if (ann.pendingConfirm && ann.pendingConfirm.length > 0) {
        html += '<div class="tl-agent-line" id="al-pc-' + msgIdx + '" style="display:none"><div class="tl-agent-row"><span class="tl-agent-icon">❓</span><div class="tl-agent-content"><span class="tl-agent-label">待确认</span>';
    ...
    // Agent 日志逐行展示 + 实时滚动到底部
    // 每次展开新行后等待浏览器 reflow 再滚动，确保新内容可见
    ...
    if (ann.agentLog) {
      ...
      if (anl) { setTimeout(function() { anl.style.display = ''; scrollToBottom(); }, delay * 1); }
      if (upd) { setTimeout(function() { upd.style.display = ''; scrollToBottom(); }, delay * 2); }
      if (act) { setTimeout(function() { act.style.display = ''; scrollToBottom(); }, delay * 3); }
      if (pc) { setTimeout(function() { pc.style.display = ''; scrollToBottom(); }, delay * 4); }
    }
