...
var _lastAnnContent = '';     // 上次渲染的注解内容，用来检测变化
  ...
  function appendTimelineCard(ann, msgIdx) {
    ...
    if (ann.agentLog) {
      ...
      if (tools && tools.length > 0) {
        var phaseLabels = {read:'READ', think:'THINK', exec:'EXEC', out:'OUT'};
        tools.forEach(function(t, si) {
          ...
          if (t.phase) {
            html += '<div class="tl-phase-divider" id="al-phase-' + msgIdx + '-' + si + '"' + style + '>' + (phaseLabels[t.phase] || t.phase) + '</div>';
          }
          html += '<div class="tl-agent-line" id="al-step-' + msgIdx + '-' + si + '"' + style + '>'
      ...
      for (var si = 1; si < total; si++) {
        (function(idx) {
          setTimeout(function() {
            ...
            if (el) { el.style.display = ''; scrollToBottom(); }
  ...
  function _toolIconSvg(name) {
    var icons = {
      ...
      'book-open': '<svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>',
    ...
    return icons[name] || '<svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/></svg>';
  }
