...
var _lastAnnContent = '';     // 上次渲染的注解内容，用来检测变化
  ...
  function appendTimelineCard(ann, msgIdx) {
    ...
    if (ann.agentLog) {
      ...
      if (anl) { setTimeout(function() { anl.style.display = ''; tlBody.scrollTop = tlBody.scrollHeight; }, delay * 1); }
      if (upd) { setTimeout(function() { upd.style.display = ''; tlBody.scrollTop = tlBody.scrollHeight; }, delay * 2); }
      if (act) { setTimeout(function() { act.style.display = ''; tlBody.scrollTop = tlBody.scrollHeight; }, delay * 3); }
      if (pc) { setTimeout(function() { pc.style.display = ''; tlBody.scrollTop = tlBody.scrollHeight; }, delay * 4); }
    }
